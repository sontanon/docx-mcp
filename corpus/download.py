...
def main() -> None:
    ...
    for i, item in enumerate(subset, 1):
        ...
        print(
            f"  {i}. {filename:40s} | {topic:15s} | {word_count:5,} words | "
    ...
    for r in valid_results:
        ...
        try:
            ...
            print(
                f"    Headers:     {audit.header_parts} parts, "
                f"{audit.header_paragraphs} paragraphs"
            )
            print(
                f"    Footers:     {audit.footer_parts} parts, "
                f"{audit.footer_paragraphs} paragraphs"
            )
            print(
                f"    Images:      {audit.images_body} body, "
                f"{audit.images_header} header, {audit.images_footer} footer"
            )
            print(
                f"    Tables:      {audit.simple_tables} simple, "
                f"{audit.skipped_tables} skipped"
            )
